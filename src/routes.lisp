...
(defun generate-sor-report-pdf ()
  "Generate and serve SOR report as PDF."
  (let* ((camp-param (or (get-param "camp") ""))
         (all-camps-p (string= camp-param "all"))
         (camp-id (unless all-camps-p (parse-int camp-param)))
         (date-from (or (get-param "date_from") ""))
         (date-to (or (get-param "date_to") ""))
         (repair-by (or (get-param "repair_by") ""))
         (status-filter (or (get-param "status") ""))
         (contract-week (or (get-param "contract_week") "")))
    (if (or camp-id all-camps-p (> (length date-from) 0))
        (let* ((summary (get-sor-summary :camp-id (unless all-camps-p camp-id)
                                         :date-from date-from :date-to date-to
                                         :repair-by (when (> (length repair-by) 0) repair-by)
                                         :status (when (> (length status-filter) 0) status-filter)))
               (by-responsibility (get-sor-by-repair-responsibility 
                                   :camp-id (unless all-camps-p camp-id)
                                   :date-from date-from :date-to date-to))
               ...
               (site-name (if all-camps-p "All Sites" (or (getf camp :|name|) "Unknown")))
               (json-data (cl-json:encode-json-to-string
                           `((:site_name . ,site-name)
                             (:contract_week . ,contract-week)
                             (:date_from . ,date-from)
                             (:date_to . ,date-to)
                             (:repair_by . ,repair-by)
                             (:status . ,status-filter)
                             (:summary . ((:total . ,(or (getf summary :|total|) 0))
                                          (:open_count . ,(or (getf summary :|open_count|) 0))
                                          (:closed_count . ,(or (getf summary :|closed_count|) 0))
                                          (:reinsp_count . ,(or (getf summary :|reinsp_count|) 0))))
                             (:by_responsibility . ,(mapcar (lambda (r)
                                                              `((:repair_by . ,(getf r :|repair_by|))
                                                                (:total . ,(getf r :|total|))
                                                                (:open_count . ,(or (getf r :|open_count|) 0))
                                                                (:closed_count . ,(or (getf r :|closed_count|) 0))
                                                                (:reinsp_count . ,(or (getf r :|reinsp_count|) 0))))
                                                            by-responsibility))
                             (:by_camp . ,(mapcar (lambda (c)
                                                    `((:camp_name . ,(getf c :|camp_name|))
                                                      (:country . ,(or (getf c :|country|) ""))
                                                      (:total . ,(getf c :|total|))
                                                      (:open_count . ,(or (getf c :|open_count|) 0))
                                                      (:closed_count . ,(or (getf c :|closed_count|) 0))
                                                      (:reinsp_count . ,(or (getf c :|reinsp_count|) 0))))
                                                  (or by-camp '())))
                             (:details . ,(mapcar (lambda (d)
                                                    `((:camp_name . ,(or (getf d :|camp_name|) ""))
                                                      (:building_number . ,(or (getf d :|building_number|) ""))
                                                      (:deficiency_number . ,(or (getf d :|deficiency_number|) ""))
                                                      (:def_category . ,(or (getf d :|def_category|) ""))
                                                      (:repair_by . ,(or (getf d :|repair_by|) ""))
                                                      (:om_sor_number . ,(or (getf d :|om_sor_number|) ""))
                                                      (:deficiency_status . ,(or (getf d :|deficiency_status|) ""))
                                                      (:rac . ,(or (getf d :|rac|) ""))
                                                      (:inspection_date . ,(or (getf d :|inspection_date|) ""))))
                                                  details)))))
               (timestamp (get-universal-time))
               (json-file (format nil "/tmp/sor-report-~A.json" timestamp))
               (pdf-file (format nil "/tmp/sor-report-~A.pdf" timestamp))
               (static-dir (namestring (merge-pathnames "static/" *base-directory*))))
          (with-open-file (out json-file :direction :output :if-exists :supersede)
            (write-string json-data out))
          (uiop:run-program 
           (list "python3" 
                 (namestring (merge-pathnames "scripts/generate_sor_pdf.py" *base-directory*))
                 json-file pdf-file static-dir)
           :output :string :error-output :string :ignore-error-status t)
          (if (probe-file pdf-file)
              (progn
                (setf (hunchentoot:content-type*) "application/pdf")
                (setf (hunchentoot:header-out :content-disposition)
                      (format nil "attachment; filename=\"SOR-Report-~A.pdf\"" 
                              (local-time:format-timestring nil (local-time:now) 
                                                            :format '(:year (:month 2) (:day 2)))))
                (with-open-file (in pdf-file :element-type '(unsigned-byte 8))
                  (let ((buffer (make-array (file-length in) :element-type '(unsigned-byte 8))))
                    ...
                    buffer)))
              (progn
                (when (probe-file json-file) (delete-file json-file))
        ...
        (:ul :class "nav-links"
          ...
          (:li :class "nav-dropdown"
            ...
            (:ul :class "nav-dropdown-menu"
              ...
              (:li (:a :href "/reports/sor" "SOR Reports"))
...
(defun dispatch-request ()
  ...
  (let* ((full-uri (hunchentoot:request-uri*))
    ...
    (cond
      ...
      ((string= uri "/reports/sor")
       (handle-sor-reports))
      ((string= uri "/reports/sor/pdf")
       (generate-sor-report-pdf))
      ...
      ((cl-ppcre:scan "^/mrf/(\\d+)/pdf$" uri)
       (multiple-value-bind (match groups) (cl-ppcre:scan-to-strings "^/mrf/(\\d+)/pdf$" uri)
         (declare (ignore match))
         (handle-mrf-pdf (aref groups 0))))
      ...
      ((cl-ppcre:scan "^/perstat/personnel/(\\d+)/edit$" uri)
       (multiple-value-bind (match groups) (cl-ppcre:scan-to-strings "^/perstat/personnel/(\\d+)/edit$" uri)
         (declare (ignore match))
         (handle-perstat-personnel-edit (aref groups 0))))
      ((cl-ppcre:scan "^/perstat/personnel/(\\d+)$" uri)
       (multiple-value-bind (match groups) (cl-ppcre:scan-to-strings "^/perstat/personnel/(\\d+)$" uri)
         (declare (ignore match))
         (handle-perstat-personnel-detail (aref groups 0))))
      ...
      ((and (eq method :post) (cl-ppcre:scan "^/api/perstat/personnel/(\\d+)/update$" uri))
       (multiple-value-bind (match groups) (cl-ppcre:scan-to-strings "^/api/perstat/personnel/(\\d+)/update$" uri)
         (declare (ignore match))
      ...
      ((and (eq method :post) (cl-ppcre:scan "^/api/rr/(\\d+)/cancel$" uri))
       (multiple-value-bind (match groups) (cl-ppcre:scan-to-strings "^/api/rr/(\\d+)/cancel$" uri)
         (declare (ignore match))
         (handle-api-rr-cancel (aref groups 0))))
